  ...
  function start(){
    ...
    try {
      ...
      reply = function reply(qRaw){
        ...
        if (intent === "ai")  return handleAiFlow(info);
        ...
        if (intent === "more"){
          ...
          if (state.topic === "ai")  return { text: "I can outline a validation plan template: dataset governance, acceptance criteria, subgroup performance, drift monitoring, failure-mode controls. What’s the intended use?", showCTA: true };
    ...
    if (form){
      form.addEventListener("submit", (e)=>{
        ...
        let r = {};
        try { r = reply(q) || {}; } catch(err){ console.error("dc-chat reply error:", err); r = { text: "Something went wrong. Try again or say contact." }; }
