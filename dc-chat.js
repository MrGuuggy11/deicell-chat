(function () {
  ...
  function start() {
    // Don't early-return on mobile; just probe (no-op)
    try { window.matchMedia && window.matchMedia("(hover:none) and (pointer:coarse)").matches; } catch (_) {}
    ...
    const setOpen = open => {
      ...
      if (open) setTimeout(() => { try { input && input.focus(); } catch (_) {} }, 0);
    };
    ...
    // ---------- State / Render ----------
    let history = [];
    try { history = JSON.parse(localStorage.getItem(LS.hist) || "[]") || []; } catch (_) { history = []; }
    ...
    const save = () => { try { localStorage.setItem(LS.hist, JSON.stringify(history)); } catch (_) {} };
    ...
    function reply(qRaw) {
      ...
      // FIX: proper dot escape in the regex
    ...
    form.addEventListener("submit", e => {
      ...
      let r = {};
      try { r = reply(q); }
      catch (err) { console.error("dc-chat reply error", err); r = { text: "Sorry. I had trouble with that. Try again.", capture: false }; }
