/* dc-chat.js — DeiCell “Mendel AI” chat widget (Carrd-safe) */
(function () {
  "use strict";

  function start() {
    // Allow mobile; keep guard but don't exit
    try {
    ...
    // Ensure root exists (do NOT set aria-hidden here — it will be toggled on panel)
    ...
    console.log("[dc-chat] boot: root ready");
    ...
    panel.setAttribute("aria-hidden", "true"); // starts closed
    btn.setAttribute("aria-controls", "dc-chat-panel");
    ...
    // Open/close with proper ARIA toggling (root remains visible to AT)
    const setOpen = (open) => {
    ...
    function addCapture() {
      ...
      setTimeout(() => {
        ...
        f.addEventListener("submit", (e) => {
          ...
          add(
            "assistant",
            sent ? "Thanks — submitted! We’ll be in touch soon." : "Thanks — saved locally. We’ll reach out soon."
          );
    ...
    function addCTA() {
      // still available for replies that set showCTA
      const html = bubble(
        "assistant",
        `
    ...
    // Knowledge & reply
    let lastTopic = null,
      regCtx = { cls: null, region: null };
    const KB = {
      nathan:
        `Nathan Jones is a regulatory and quality systems consultant and the Founder of DeiCell Systems. ` +
        `He helps biotech and medtech teams build inspection ready operations across GMP, ISO 13485, and ICH, ` +
        `bridging bench biology with scalable compliance from development through postmarket. Hands on with batch review, ` +
        `CAPA, labeling compliance, and USP <61>/<62>/<71> microbiology. Completing an MBA at Xavier and pursuing RAC through RAPS.`,
      deicell:
        `DeiCell Systems helps biotech and medtech innovators scale with precision. We provide regulatory, quality, ` +
        `and strategic consulting grounded in systems thinking and scientific rigor, from GxP compliant frameworks to ` +
        `microbiology informed compliance strategies, for early and growth stage teams.`,
      value:
        `Why DeiCell: risk based, right sized systems; traceable, audit ready documentation; microbiology aware controls; ` +
        `and pragmatic coaching so teams stay fast without breaking compliance.`,
      qms_intro:
        `QMS: right sized ISO 13485 architecture, doc control, training, CAPA, risk, change control, plus audit prep and stage appropriate SOPs.`,
      qms_startup:
        `For startups: lean, tiered QMS that scales with funding and maturity. We emphasize essentials like risk, doc control, and training, ` +
        `and deepen as you approach clinical or market milestones.`,
      reg_intro:
        `Regulatory: pathway and testing strategy, Pre Sub planning, submission authoring such as 510(k), De Novo, PMA, and labeling reviews aligned to FDA, ISO, and MDR.`,
      reg_class3_us:
        `US Class III: PMA strategy, Pre Sub questions, clinical evidence planning, design dossier structure, biocompatibility and statistics pointers, labeling checks, plus inspection readiness and traceability.`,
      ai_intro:
        `AI and ML: data governance, validation planning, model risk checks, and evidence packages. We pair regulatory insight with NLP assisted review for literature, PMS, and training datasets.`,
    };
    ...
    function reply(qRaw) {
      ...
      if (emailMatch) {
        ...
        return {
          text: `Thanks, noted ${emailMatch[0]}. Add a note and hit Submit Info if you like.`,
          showCTA: true,
          showContact: true,
        };
      }
      if (askWhoNJ) {
        lastTopic = "nathan";
        return { text: KB.nathan, showContact: true };
      }
      if (askDei || has(q, ["what do you do", "tell me more about your company"])) {
        lastTopic = "deicell";
        return { text: `${KB.deicell}\n\n${KB.value}`, showCTA: true, showContact: true };
      }
      if (askWhy) {
        lastTopic = "deicell";
        return { text: KB.value, showCTA: true, showContact: true };
      }
      if (askEmail) {
        if (!document.getElementById("dc-capture")) addCapture();
        return {
          html: `Use the form above or email <a href="mailto:${COMPANY_EMAIL}" style="color:var(--dc-link)">${COMPANY_EMAIL}</a>.`,
          showCTA: true,
        };
      }
      if (askLinked) {
        return {
          html: `Company: <a href="${COMPANY_LINKEDIN}" target="_blank" rel="noopener" style="color:#0aa2ff">LinkedIn</a><br>${FOUNDER_NAME.split(
            ","
          )[0]}: <a href="${FOUNDER_LINKEDIN}" target="_blank" rel="noopener" style="color:#0aa2ff">LinkedIn</a>`,
        };
      }
      if (qms) {
        lastTopic = "qms";
        return { text: `${KB.qms_intro}\n\nWhat is your stage and target standard?` };
      }
      if (reg) {
        lastTopic = "regulatory";
        regCtx = { cls: null, region: null };
        return { text: `${KB.reg_intro}\n\nWhat device class and region?` };
      }
      if (ai) {
        lastTopic = "ai";
        return { text: `${KB.ai_intro}\n\nWhat model or use case are you qualifying?` };
      }
      if (startup) {
        if (lastTopic === "qms" || lastTopic === null) {
          lastTopic = "qms";
          return { text: KB.qms_startup, capture: true, showCTA: true };
        }
        if (lastTopic === "regulatory") {
          return {
            text:
              `For early teams, clarify intended use and claims, line up biocomp and bench testing, and plan a Pre Sub. ` +
              `We can draft questions and outline evidence so you do not over or under test.`,
            capture: true,
            showCTA: true,
          };
        }
        if (lastTopic === "ai") {
          return {
            text:
              `For AI startups, define data lineage, validation acceptance criteria, and risk controls up front. ` +
              `We can help assemble an evidence package you can reuse for audits.`,
            capture: true,
            showCTA: true,
          };
        }
      ...
      if (lastTopic === "regulatory") {
        ...
        if (regCtx.cls && regCtx.region) {
          ...
          if (regCtx.cls === "Class II" && regCtx.region === "US") {
            return {
              text:
                `${both}: Likely 510(k). We will align intended use and claims, evaluate predicate strategy, ` +
                `scope bench and biocomp testing, and prep labeling for submission. We can also draft a focused Pre Sub if novel.`,
              capture: true,
              showCTA: true,
              showContact: true,
            };
          ...
          if (regCtx.region === "EU") {
            return {
              text:
                `${both}: Plan MDR conformity with the right GSPRs, clinical evaluation strategy, and PMS or PMCF. ` +
                `We can outline technical documentation and gap check labeling.`,
              capture: true,
              showCTA: true,
              showContact: true,
            };
        ...
        if (!regCtx.cls && (inUS || inEU))
          return { text: `Region set to ${regCtx.region || "US or EU"}. What device class, II or III?` };
      }
      ...
      return {
        text: `Happy to help. Is this about QMS, Regulatory, or AI? I can also connect you to a consultant.`,
        capture: true,
      };
    }
    ...
    if (history.length) {
      ...
      add("assistant", `Hi — I'm ${BOT_NAME}. Ask a quick question or say contact if you want to reach a consultant.`);
    }
    ...
    function showWelcomePrompt() {
      ...
      const msg = hasUser
        ...
        : "Are you looking for any help? I may be able to assist.";
      ...
      node.querySelectorAll("[data-act]").forEach((b) => {
        b.addEventListener("click", (e) => {
          ...
          if (act === "continue") {
            ...
            add(
              "assistant",
              "Great. Starting fresh. Ask a quick question or say contact if you want to reach a consultant."
            );
            showWelcomePrompt();
          } else if (act === "yes") {
            node.remove();
            add(
              "assistant",
              "Happy to help. Is this about QMS, Regulatory, or AI? I can also connect you to a consultant."
            );
          } else if (act === "close") {
            close();
          }
    ...
    setOpen(true);
    showWelcomePrompt();
    console.log("[dc-chat] UI mounted & opened");
  }
